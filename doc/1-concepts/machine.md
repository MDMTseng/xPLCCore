...
目標是盡量讓動作重疊，壓縮 cycle time。兩條時序最需要注意：

**時序一：柔震盤補料，目標約 1 秒**

最後一顆可取的料從柔震盤被取走 → 震動電磁閥強制停止 → 一號燈打光、柔震頂部相機拍照 → 可辨識的料太少就補料（震動）→ 靜止後再拍。與手臂前往檢驗站、載帶站的動作重疊；手臂回到柔震盤前，下一顆料的位置要準備好。

**時序二：載帶前進與雙光拍照，目標 0.7 秒內**

放完料、Z 軸上升 → 載帶前進 N 格 → 四號燈、五號燈各拍一張。與手臂回柔震盤取料、檢驗的動作重疊；結果要在下一次放料前完成。

可壓縮的地方（尚未實作）：

- 柔震盤震動目前由電腦經 USB-485 控制，多了電腦端的延遲和抖動；改由 PLC 控制（RS-485 修好，或經 EasyCAT/ESP32），可用飛拍事件在精確時間點觸發。
- 柔震盤拍照可在手臂離開柔震盤上方的瞬間（飛拍事件）觸發，不必等整段移動結束。
- 載帶拍照改以 `WAIT_FOR_REEL_STOP` 加手臂位置觸發，取代固定延遲（見下節）。
- 先量測：在 PLC 記錄各關鍵事件的時間戳，畫成時間軸，找出最慢的一段再動手。

### 拍照時機的現況（2026-09-24）

- 「手臂已離開視野」和「載帶前進完成」目前都是**固定延遲**，調整到實際不會拍到手臂為止。PLC 也有飛拍事件（`M4` 依移動進度觸發）可以用來觸發。
- 延遲的做法來自舊機器（本專案第一版）：舊機器的載帶機和 PLC 是分開的，只能送「前進」指令，無法得知何時前進完成，只能等延遲。
- 這台新機器的載帶軸 `reelpullmotor` 就在 PLC 的 EtherCAT 上，可以直接知道前進何時完成（例如 `WAIT_FOR_REEL_STOP`），手臂位置也可以用飛拍事件判斷，所以可以改成依實際狀態觸發，不必靠延遲。尚未實作。
- Z 軸旋轉的實體軸：使用者認為是 `EAXIS_A`，尚未從 `SpiderR` 軸群組設定確認。
