...
| `WAIT_FOR_REEL_STOP` / `BLOCK_FOR_REEL_STOP` | `timeout_ms?` (LINT, ms) | `{ack:true}` | Wait until the **reel axis** is idle (it is NOT in the delta group, so `WAIT_FOR_MOTION_STOP` doesn't cover it). Deferred-ack, not a scan-spin: the PLC holds the ack frame and re-checks each scan (`reelGoRequest*` OR `reelMoveRelative*.Busy`), acking once idle. Lets the host `await` a `ReelGo` without polling `reel_pos`. `0`/absent = wait forever; NAK `block_timeout` on expiry. |
