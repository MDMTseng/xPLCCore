...
| `SET_AXIS_SIM` | `mask` (uint 0–15) | `{ack:true, mask}` | Per-axis simulation mask (bench mode). Bit layout matches `axes_labels` order: bit 0–2 = `EAxis0/1/2` (delta trio), bit 3 = `reelpullmotor`; 1 = simulated. Honored **only while FSM=UnInited** (drives are powerless there, so flipping virtual mode is safe) — NAK `not_uninited` otherwise; NAK `bad_mask` for out-of-range. Reply echoes the **effective** mask (request OR `0x0F` when the legacy `GVL.bVirtualMotorsMode` gate is open — the daemon-forced all-axes flag stays fully backward compatible). The effective mask self-expires after 10 min (same TTL policy as the legacy gate) — re-issue `SET_AXIS_SIM` after a later `EV_RESET` if the session outlives it. Current mask is published as `axes_sim_mask` in `GET_MACHINE_STATE`. |
