...
| `PLAN_SET` / `PLAN_GET` | see `lib/protocol.ts` | | The production plan kept by the PLC (retained): the whole plan and the tape cells advanced. `PLAN_GET` also reports an open tape move: `reel_open`, `reel_moving`, `reel_pos_lost`, `reel_rest_mm`, `reel_cells`, `reel_counted`, `reel_interrupts` ([estop_recovery](../../doc_review/estop_recovery_2026-09-25.md)). |
...
| `DI` | `{pin, state, flips, high_ms, max_high_ms, max_low_ms, t_first, reel_pos, t}` | A pin registered with `DI_WATCH` changed (see there). `state`: level now; `flips`: changes since the pin's last event; `high_ms`: time high since then; `max_high_ms`/`max_low_ms`: longest run at each level that ended since then or is still going; `t_first`/`reel_pos`: PLC `RuntimeMs` and reel position at the first of those changes; `t`: `RuntimeMs` at the push. |
