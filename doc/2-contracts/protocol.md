...
| `GA_EV` | `ev` (int — `E_RobotEvent` ordinal) | `{st, st_str, err_src, err_id}` | Drive FSM transition. Host-postable events are **exactly** `{2,4,6,7,8,9}` (`EV_POWER_ON`/`EV_GROUP_ENABLE`/`EV_HOME_GO`/`EV_HOME_GO_FORCE_SKIP`/`EV_RESET`/`EV_ERROR`); anything else — including the sparse-enum gaps 1/3/5 and the internal `EV_OK`=10/`EV_ER`=11 — NAKs `unknown_event`. SYS-only. See [memory: E_RobotEvent numeric values](../../.claude/projects/c--Users-X1-Desktop-X2-5-TCP-UI-TCP-UI/memory/plc_event_numeric_values.md). |
